...
- [目錄](#目錄)
- [子視窗、工具視窗實作](#子視窗工具視窗實作)
  - [設計原則（工控 + 低效能環境）](#設計原則工控--低效能環境)
  - [1️⃣ 模態視窗（阻斷流程型）](#1️⃣-模態視窗阻斷流程型)
    - [特性](#特性)
    - [Form 設計](#form-設計)
    - [呼叫端寫法（推薦標準寫法）](#呼叫端寫法推薦標準寫法)
  - [2️⃣ 工具 / 監控視窗（非阻斷型）](#2️⃣-工具--監控視窗非阻斷型)
  - [種類 1：每次關閉都 Dispose](#種類-1每次關閉都-dispose)
    - [特性](#特性-1)
    - [Form 設計](#form-設計-1)
    - [主畫面管理](#主畫面管理)
  - [種類 2：Show / Hide，不 Dispose](#種類-2show--hide不-dispose)
    - [主畫面管理](#主畫面管理-1)
  - [工控強化建議](#工控強化建議)
    - [1️⃣ 防止重複開啟多個視窗](#1️⃣-防止重複開啟多個視窗)
    - [2️⃣ 防止跑出螢幕外（雙螢幕/拔掉螢幕）](#2️⃣-防止跑出螢幕外雙螢幕拔掉螢幕)
  - [工控環境選擇建議](#工控環境選擇建議)
- [WinForms（.NET Framework 4.8）由按鍵開啟／關閉 Form 的正確寫法](#winformsnet-framework-48由按鍵開啟關閉-form-的正確寫法)
  - [一、非模態視窗（可同時操作主視窗）推薦實務寫法](#一非模態視窗可同時操作主視窗推薦實務寫法)
    - [設計原則](#設計原則)
    - [範例](#範例)
  - [二、模態視窗（ShowDialog）](#二模態視窗showdialog)
    - [範例](#範例-1)
  - [三、只 Hide 不 Close（保留狀態）](#三只-hide-不-close保留狀態)
    - [範例](#範例-2)
  - [四、架構層級建議（避免常見錯誤）](#四架構層級建議避免常見錯誤)
  - [簡潔總結](#簡潔總結)
- [WinForms 多視窗管理：Form Manager + Factory 架構（.NET Framework 4.8）](#winforms-多視窗管理form-manager--factory-架構net-framework-48)
  - [架構概念](#架構概念)
  - [一、IFormFactory](#一iformfactory)
  - [二、基本 Factory 實作](#二基本-factory-實作)
  - [三、IFormManager](#三iformmanager)
  - [四、FormManager 實作（核心）](#四formmanager-實作核心)
  - [五、MainForm 使用方式](#五mainform-使用方式)
  - [六、進階：改為 Hide 模式（保留狀態）](#六進階改為-hide-模式保留狀態)
  - [七、工控強化建議](#七工控強化建議)
  - [八、延伸：多設備多實例版本（依 Key 管理）](#八延伸多設備多實例版本依-key-管理)
- [FormManager 會增加工控環境負擔嗎？](#formmanager-會增加工控環境負擔嗎)
  - [記憶體負擔](#記憶體負擔)
  - [CPU 負擔](#cpu-負擔)
    - [穩定性角度（反而是優點）](#穩定性角度反而是優點)
- [團隊學習曲線與排斥問題](#團隊學習曲線與排斥問題)
  - [若設計成過度工程化（會被排斥）](#若設計成過度工程化會被排斥)
  - [正確的工控版 FormManager 應該長這樣](#正確的工控版-formmanager-應該長這樣)
  - [實務團隊接受度分析](#實務團隊接受度分析)
    - [會排斥的情況](#會排斥的情況)
    - [容易接受的情況](#容易接受的情況)
  - [工控專案真實風險比較](#工控專案真實風險比較)
    - [沒有管理器的長期風險](#沒有管理器的長期風險)
    - [有簡單 FormManager 的風險](#有簡單-formmanager-的風險)
  - [建議結論](#建議結論)
- [視窗開關管理有那麼多問題嗎？](#視窗開關管理有那麼多問題嗎)
  - [簡短結論](#簡短結論)
  - [為什麼一般桌面軟體沒事？](#為什麼一般桌面軟體沒事)
  - [為什麼工控 24/7 會出問題？](#為什麼工控-247-會出問題)
  - [實務常見問題類型](#實務常見問題類型)
    - [1️⃣ GDI 漏洞（最常見）](#1️⃣-gdi-漏洞最常見)
    - [2️⃣ Handle 累積](#2️⃣-handle-累積)
    - [3️⃣ UI Freeze](#3️⃣-ui-freeze)
    - [4️⃣ 記憶體慢性上升](#4️⃣-記憶體慢性上升)
  - [那是不是一定要做那麼重？](#那是不是一定要做那麼重)
    - [如果系統條件是](#如果系統條件是)
    - [如果系統條件是](#如果系統條件是-1)
  - [真實經驗數據](#真實經驗數據)
  - [重點不是「視窗管理」](#重點不是視窗管理)
  - [你真正要問的是](#你真正要問的是)
  - [工控分級建議](#工控分級建議)
    - [Level 1（基礎）](#level-1基礎)
    - [Level 2（穩定）](#level-2穩定)
    - [Level 3（24/7）](#level-3247)
  - [我會反問你一個實務問題](#我會反問你一個實務問題)
  - [最務實結論](#最務實結論)
- [自訂繪圖專案專用的 24/7 WinForms 工控框架模板](#自訂繪圖專案專用的-247-winforms-工控框架模板)
  - [一、FormManager（簡化版）](#一formmanager簡化版)
  - [二、Safe UI Invoker](#二safe-ui-invoker)
  - [三、UI Freeze 監控](#三ui-freeze-監控)
  - [四、資源監控（GDI / Handle / Memory）](#四資源監控gdi--handle--memory)
  - [五、穩定性監控整合](#五穩定性監控整合)
  - [六、全域例外隔離](#六全域例外隔離)
  - [七、主程式整合範例](#七主程式整合範例)
    - [特點](#特點)
