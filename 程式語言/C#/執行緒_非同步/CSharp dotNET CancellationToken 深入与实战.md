...
author: 唐青枫
language: C#
sourceurl: https://zhuanlan.zhihu.com/p/1938871920603931807
tags:
  - CSharp
  - 非同步程式設計
---
...
var cts = new CancellationTokenSource(5000); // 5 秒超时
...
```csharp
await HttpClient.GetAsync(url, token); // API 内部会调用 ThrowIfCancellationRequested
```
...
| 高频调用点（>1000 次/秒） | `IsCancellationRequested`      |
